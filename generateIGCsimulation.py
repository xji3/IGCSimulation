...
# pretend I am not doing this...

    IGC_geo_list = [1.0, 10.0, 50.0, 100.0, 500.0]
    for IGC_geo in IGC_geo_list[0:1]:
        #IGC_geo  = 1.0
        IGC_init = tau / IGC_geo  # count for three nucleotides in a codon
        x_IGC = [IGC_init, 1.0 / IGC_geo, IGC_threshold]  # These values vary for the simulation study
        
        #sim_num = 1
        for sim_num in [72]:#range(100):
            log_folder = './' + '_'.join(paralog) + '_multiply3/IGCgeo_' + str(IGC_geo) + '/sim_' + str(sim_num) + '/log/'
            div_folder = './' + '_'.join(paralog) + '_multiply3/IGCgeo_' + str(IGC_geo) + '/sim_' + str(sim_num) + '/div/'

            # Now create folder if they don't exist
            
            if not os.path.isdir('./' + '_'.join(paralog) + '_multiply3/IGCgeo_' + str(IGC_geo) + '/'):
                os.mkdir('./' + '_'.join(paralog) + '_multiply3/IGCgeo_' + str(IGC_geo) + '/')

            if not os.path.isdir('./' + '_'.join(paralog) + '_multiply3/IGCgeo_' + str(IGC_geo) + '/sim_' + str(sim_num) + '/'):
                os.mkdir('./' + '_'.join(paralog) + '_multiply3/IGCgeo_' + str(IGC_geo) + '/sim_' + str(sim_num) + '/')
            ...
            seq_file = './' + '_'.join(paralog) + '_multiply3/IGCgeo_' + str(IGC_geo) + '/sim_' + str(sim_num) + '/' + '_'.join(paralog) + '_MG94_geo_' + str(IGC_geo) + '_Sim_' + str(sim_num) + '.fasta'
            log_file = './' + '_'.join(paralog) + '_multiply3/IGCgeo_' + str(IGC_geo) + '/sim_' + str(sim_num) + '/' + '_'.join(paralog) + '_MG94_geo_' + str(IGC_geo) + '_Sim_' + str(sim_num) + '.log'
